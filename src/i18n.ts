...
const MESSAGES = {
  en: {
    ...
    cmd_desc_backend: 'Switch backend engine and account',
    ...
    cmd_desc_login_cancel: 'Cancel the pending login',
    cmd_desc_cli: 'Continue this session in the local CLI',
    ...
    auth_sync_safe_starting: 'Auditing every node and distributing valid auth. An unreachable peer can add up to 5 minutes per audit phase. Use /auth sync status to check progress.',
    ...
    auth_cluster_audit_synced: 'Valid auth selected and sent (remote import not confirmed): {value}',
    ...
    usage_takeover: 'Usage: /takeover <message> or /takeover --force <message> (confirm stopping a local CLI)',
    force_takeover_trusted_telegram_only: 'Force takeover is only available to the configured trusted Telegram user.',
    force_takeover_confirm: 'Force takeover of thread {threadId}?\nLocal Codex CLI PID: {pid}\nWorking directory: {cwd}\nNext prompt: {prompt}\nThis will terminate that CLI (SIGTERM, then SIGKILL after 5 seconds if needed). Unfinished work may be interrupted; child commands may continue running. No edits are rolled back. Confirmation expires in 60 seconds.',
    button_force_takeover: 'Confirm force takeover',
    force_takeover_busy: 'Force takeover is in progress. Please wait before switching threads.',
    force_takeover_hint: 'Use /watch then /queue <message> to queue for that CLI. To stop a local CLI and hand the thread to this bridge, use /takeover --force <message> and confirm.',
    force_takeover_expired: 'Confirmation expired or thread changed. Send /takeover --force <message> again.',
    force_takeover_cancelled: 'Force takeover cancelled. Nothing was stopped or submitted.',
    force_takeover_stopping: 'Stopping the confirmed CLI PID {pid} and checking its writer lock...',
    force_takeover_failed: 'Force takeover not completed; no new prompt was sent. If stopping already began, the CLI may have exited. Check /status before retrying. Reason: {error}',
    force_takeover_acquired: 'Bridge acquired thread {threadId}. Preparing the requested prompt; this is not task completion.',
    ...
    watch_read_only_active: 'This chat is watching that turn read-only, so this operation cannot take ownership. Send plain text or use /queue to queue the next prompt, or /unwatch to start separately.',
    watch_prompt_queued: 'Queued for the watched Codex CLI session ({id}). The CLI will continue with it after the current turn.',
    watch_queue_unsupported: 'This Codex version does not support cross-client session queues. Update Codex to 0.151.0 or later.',
  ...
  zh: {
    ...
    cmd_desc_login_cancel: '取消当前登录流程',
    cmd_desc_cli: '从本机 CLI 继续当前会话',
    ...
    auth_sync_safe_starting: '正在自检各节点并分发有效授权。节点未回应时，每轮自检最多等待 5 分钟；可用 /auth sync status 查看进展。',
    ...
    auth_cluster_audit_synced: '已选出并发送有效授权（远端导入尚未确认）：{value}',
    ...
    usage_takeover: '用法：/takeover <消息>；或 /takeover --force <消息>（确认后停止本机 CLI）',
    force_takeover_trusted_telegram_only: '强制接管仅对配置中的可信 Telegram 用户开放。',
    force_takeover_confirm: '强制接管 thread {threadId}？\n本机 Codex CLI PID：{pid}\n工作目录：{cwd}\n接管后消息：{prompt}\n将终止这个 CLI（先 SIGTERM，5 秒后仍未退出则 SIGKILL）。未完成任务可能被中断，已启动的子命令可能继续运行；不会回滚文件修改。确认 60 秒内有效。',
    button_force_takeover: '确认强制接管',
    force_takeover_busy: '正在强制接管，请等待结果后再切换会话。',
    force_takeover_hint: '可先 /watch 再 /queue <消息> 排队给原 CLI；如需停止本机 CLI、让桥接管，请发送 /takeover --force <消息> 并确认。',
    force_takeover_expired: '确认已过期或 thread 已切换，请重新发送 /takeover --force <消息>。',
    force_takeover_cancelled: '已取消强制接管，没有停止进程或发送新任务。',
    force_takeover_stopping: '正在停止已确认的 CLI PID {pid}，并检查写入锁是否释放…',
    force_takeover_failed: '强制接管未完成，没有发送新任务。如果已开始停止，CLI 可能已经退出。请先 /status 检查再重试。原因：{error}',
    force_takeover_acquired: '桥已取得 thread {threadId} 的写入权，准备发送指定消息；这不代表任务已完成。',
    ...
    watch_read_only_active: '当前聊天正在只读观察这轮，不能接管这项操作。直接发送文字或使用 /queue 可排入下一轮；要另行启动请先 /unwatch。',
    watch_prompt_queued: '已排入正在观察的 Codex CLI 会话（{id}）。当前轮结束后，CLI 会自动接着执行。',
    watch_queue_unsupported: '当前 Codex 不支持跨客户端会话队列，请升级到 Codex 0.151.0 或更高版本。',
...
export function getTelegramCommands(locale: AppLocale): Array<{ command: string; description: string }> {
  return [
    ...
    { command: 'backend', description: t(locale, 'cmd_desc_backend') },
    ...
    { command: 'boost', description: locale === 'zh' ? '切换 Boost 深度推理模式' : 'Toggle Boost reasoning mode' },
    ...
    { command: 'login_cancel', description: t(locale, 'cmd_desc_login_cancel') },
    { command: 'cli', description: t(locale, 'cmd_desc_cli') },
...
export function getAntigravityTelegramCommands(locale: AppLocale): Array<{ command: string; description: string }> {
  return locale === 'zh'
    ? [
        ...
        { command: 'backend', description: '切换后端运行环境与账号' },
    ...
    : [
        ...
        { command: 'backend', description: 'Switch backend and account' },
